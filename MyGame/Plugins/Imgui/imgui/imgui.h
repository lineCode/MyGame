...
struct ImVec3
{
	float x, y,z;
	ImVec3() { x = y = z= 0.0f; }
	ImVec3(float _x, float _y) { x = _x; y = _y; z = 0; }
#ifdef IM_VEC2_CLASS_EXTRA          // Define constructor and implicit cast operators in imconfig.h to convert back<>forth from your math types and ImVec2.
	IM_VEC2_CLASS_EXTRA
#endif

		ImVec3 & operator = (const ImVec2 &tmp)
	{
		x = tmp.x;
		y = tmp.y;
		z = 0;
	...
	operator const ImVec2() const { return ImVec2(x,y); }
};
...
struct ImFontAtlas
{
	...
	IMGUI_API const ImWchar*    GetGlyphRangesChina();    // Basic Latin, Extended Latin
